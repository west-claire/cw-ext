sap.ui.define([
    ...
    return PluginControllerExtension.extend("sap.ext.sample.downtimeExtensionProvider.PluginEventExtension", {
        ...
        onGetReasonCodeDialogFragment: function(sId, sFragment, oContext) {
            ...
            return Fragment.load({
                id: sId,
                name: "sap.dm.dme.oeeplugins.downtimePlugin.extension.CustomReasonCodeDialog",
                controller: this
            });
        },

        onReasonCodeDialogOpen: function() {
            let oReasonCodeDialog = this.getController().getReasonCodeSelectionDialog();
            if (!this.mExtensionData) {
                this.mExtensionData = new sap.ui.model.json.JSONModel();
                oReasonCodeDialog.setModel(this.mExtensionData, "extensionData");
            }
            if (!this.mBreadcrumbs) {
                this.mBreadcrumbs = new sap.ui.model.json.JSONModel([ oTopLevelBreadcrumb ]);
                oReasonCodeDialog.setModel(this.mBreadcrumbs, "breadcrumbs");
            }
        },
